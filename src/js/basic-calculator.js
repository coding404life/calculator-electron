...
let drawBtnGrid = function (options) {
  ...
  for (let i = 0; i < btnArr.length; i++) {
    ...
    if (
      btnArr[i].type == 'dot' ||
      btnArr[i].type == 'percent' ||
      btnArr[i].type == 'sqroot' ||
      btnArr[i].type == 'reciprocal' ||
      btnArr[i].type == 'clear'
    ) {
      _btn_value = btnArr[i].value
    }
    //if (btnArr[i].mode==cMode) {
    _calc_main_body.innerHTML +=
      '<button style="' +
      _btn_style +
      '" class="w3-button ' +
      btnArr[i].type +
...
let refreshHistoryTab = function () {
  ...
  if (!_anyPills.length) {
    document.querySelector('#historyTab').innerHTML =
      '<span id="emptyPill">There\'s no history yet</span>'
  } else {
